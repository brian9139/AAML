...
//   return result;
  ResultF softmax_result = FixedPoint<tRawType, 0>::FromRaw(cfu_op1(1, a.raw(), 0)); 
...
FixedPoint<tRawType, 0> logistic_on_positive_values(
    FixedPoint<tRawType, tIntegerBits> a) {
  typedef FixedPoint<tRawType, 0> ResultF;
  // Compute exp_on_negative_values(-a) then one_over_one_plus_x_for_x_in_0_1 using CFU
  ResultF logistic_result = FixedPoint<tRawType, 0>::FromRaw(cfu_op0(0, (-a).raw(), 0)); 
